...
describe("stage7SourceRegistry", () => {
  it("contains primary and manual first-iteration sources", () => {
    ...
    expect(secondQueueSourceDefinitions().map((source) => source.source_code)).toEqual([
      "edisclosure",
      "t_invest",
      "sec_edgar",
      "fed_press",
  ...
  it("records terms, rate limits and production access for every source", () => {
    expect(stage7SourceRegistry.every((source) => /^\d{4}-\d{2}-\d{2}$/.test(source.terms_checked_at))).toBe(true);
    ...
    expect(sourceDefinitionByCode("sec_edgar")).toMatchObject({ default_terms_status: "approved", automated_ingestion: true });
    expect(sourceDefinitionByCode("ecb_blog")).toMatchObject({ default_terms_status: "approved", automated_ingestion: true, trust_level: "primary" });
  });
});
